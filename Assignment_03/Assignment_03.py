...
class Order:              # has-a Customer, has-many OrderItem
    def __init__(self, customer, items):
        ...
        if type(customer) is Customer:
            raise TypeError("customer must be a concrete customer tier")
    ...
    def discount(self):
        return (
            self.customer.membership_discount(
                self.subtotal()
            )
            + self.bulk_discount()
        )
    ...
    def points(self):
        return self.customer.points_for(
            self.total()
        )
...
def build_products():
    return [
        Product(
            name,
            price,
            CATEGORY_TAX_RATES[category],
        )
        for name, price, category in PRODUCTS
    ]


def build_orders(products):
    orders = []

    for customer_name, tier, legacy_items in ORDERS:
        customer = CUSTOMER_TYPES[tier](
            customer_name
        )
